...
```
Plot $u_t$ against $y_t$ for a DCS-t-location, a Beta-t-GARCH and a Beta-t-GARCH processes for different values of $\nu = 3, 10, 200$.

In order to visualize better how the response of the score changes for different extreme observations I'm going to use the shiny library to make the plot interactive.


a) For a DCS-t location model we are going to define a function for the martingale difference sequence, the function that generates numbers from a t-student distribution and a function that simulate the model using the function provided in the R-score files uploaded in virtuale.
...
Now we are going to define the function for the the random generating numbers
...
```
Here we define the function that simulate the process, the innovations factor and the updated filters.
In particular recall that

$\mu_{t+1 | t} = \omega + phi * \mu_{t|t-1} + k*u_t $
...
```
As we can see as $\nu$ increases, $u_t$ show a more pronounced response to extreme observations. When $\nu$ is smaller, the extreme observations has a lower impact on the score.


(b) For a Beta-t-GARCH as we did before we are going to introduce the three functions but with different equations.

Here the martingale difference sequence that we are going to use is the following:
$u_t = \frac{(\nu + 1) * y^2}{(\nu-2)*\sigma_t + y^2} -1$
...
Now the data generating process of $y_t$ from a $t$-Student distribution, recall that for a Beta-t-GARCH process
...
And finally the simulator for the scale.
Recall that 
...
c) For the Beta-t-EGARCH process the MDS we are going to use is the following:
...
```
The simulation with initial number of observations and the values of the parameters
...
The second request asked to simulate a score driven model for the scale with parameter specified as I choose. 
So, let's take a GARCH (1,1) model with standard normal residuals.
...
Now to estimate the models I'm going to use the function defined in the R-score file uploaded in virtuale.
